...
class AIService {
  ...
  async processSignals(signals: Signal[]): Promise<AIServiceResponse> {
    if (this.useMockData) {
    ...
    try {
      ...
      console.error('AI API request failed, using mock data:', error);
      return this.getMockResponse(signals);
    }
  }
  ...
  async predictRisk(region: string, timeframe: string): Promise<any> {
    ...
    try {
      ...
      console.error('AI API prediction failed, using mock data:', error);
      return this.getMockPrediction(region, timeframe);
    }
  }
  ...
  async analyzeDrivers(data: any): Promise<any> {
    ...
    try {
      ...
      console.error('AI API driver analysis failed, using mock data:', error);
      return this.getMockDriverAnalysis();
    }
  }
  ...
  async recommendInterventions(region: string, riskProfile: any): Promise<any> {
    ...
    try {
      ...
      console.error('AI API intervention recommendation failed, using mock data:', error);
      return this.getMockInterventions(region);
    }
  }

  // Mock data methods for development
  private getMockResponse(signals: Signal[]): AIServiceResponse {
    return {
      assessment: {
        threat_level: 67,
        confidence: 0.85,
